...
func SetExpectations(tb *TestBackend, t *testing.T, title string, expected []WorkData) {
	tb.t = t
...
func (b *TestBackend) EmitFootnote(note string) {
	expected, ok := b.Expected[0].(Footnote)
...
func (b *TestBackend) EmitPunctuation(p string) {
	expected, ok := b.Expected[0].(Punctuation)
...
func (b *TestBackend) EndBlockQuote() {
	expected, ok := b.Expected[0].(Formatting)

	if !ok {
		b.t.Errorf("Expected %s, got EndBlockQuote.", expected)
	} else {
		if !expected.Start && (expected.Formatting == BlockQuote) {
			return
		} else {
			b.t.Errorf("Expected %s, got EndBlockQuote.", expected)
		}
...
func (b *TestBackend) EndBold() {
	...
	if !ok {
		...
		if !expected.Start && (expected.Formatting == Bold) {
			return
		} else {
			b.t.Errorf("Expected %s, got EndBold.", expected)
		}
...
func (b *TestBackend) EndItalic() {
	...
	if !ok {
		...
		if !expected.Start && (expected.Formatting == Italic) {
			return
		} else {
			b.t.Errorf("Expected %s, got EndItalic.", expected)
		}
...
func (b *TestBackend) EndParagraph() {
	...
	if !ok {
		...
		if !expected.Start && (expected.Formatting == Paragraph) {
			return
		} else {
			b.t.Errorf("Expected %s, got EndParagraph.", expected)
		}
...
func (b *TestBackend) NewLine() {
	...
	if !ok {
		...
		if (expected.Formatting == NewLine) {
			return
		} else {
